{
	"translatorID": "aa764e90-2cc7-41ee-a472-afa942b64d7e",
	"label": "Refugee Council of Australia",
	"creator": "Joyce",
	"target": "https?://(www.)?refugeecouncil.org.au*",
	"minVersion": "3.0",
	"maxVersion": "",
	"priority": 100,
	"inRepository": true,
	...
	"lastUpdated": "2020-05-31 08:19:19"
}

/*
	***** BEGIN LICENSE BLOCK *****

	Copyright © 2020 JOYCE_CHIA
...
function detectWeb(doc, url) {
	...
	if (doc.body.className.indexOf("search-results")>-1 || doc.body.className.indexOf("page-template")>-1 
			&& getSearchResults(doc, true)) {
		return "multiple";
	}
	else if (ZU.xpathText(doc, '//meta[@property="article:section" and @content="Reports"]/@content')) {
	return 'report';
}
	else if (ZU.xpathText(doc, '//meta[@property="article:section" and @content="Media"]/@content')) {
	return 'report';
}
	else {
		return 'blogPost';
...
function getSearchResults(doc, checkOnly) {
	...
	var rows = doc.querySelectorAll('h2.entry-title.h3 a');
	for (let row of rows) {
...
function scrape(doc, url) {
	...
	var pdf = ZU.xpath(doc, '//div[@class="wpmf_mce-wrap"]/a[@class="wpmf-defile wpmf_mce-single-child" and @href]')[0];
		if (pdf) {
		...
		item.title = doc.querySelector('.entry-title').textContent;
		item.publicationTitle = "Refugee Council of Australia";
		item.archiveLocation = "";
		item.libraryCatalog = "";
		item.publisher = "Refugee Council of Australia";
		item.section = "";
		item.label = "";
		item.websiteType = "";
		item.letterType = "";
		...
		item.rights = "Creative Commons 4.0 Attribution Share-Alike";
		item.tags = ZU.xpathText(doc, '//meta[@property="article:tag"]/@content').split(",");
		item.reportType = ZU.xpathText(doc, '//meta[@property="article:section"]/@content'); 
		item.complete();
	});
